...
```go include go/bxtree/bxtree.go L550-L563
	if n.isLeaf {
		mid := len(n.items) / 2
...
```go include go/bxtree/bxtree.go L614-L622
	parent := n.parent
...
```go include go/bxtree/bxtree.go L673-L682
...
```go include go/bxtree/bxtree.go L658-L671
	if length == 0 {
		return nil
	}
...
```go include go/bxtree/bxtree.go L743-L775
	var nb *Node[T, S]
	if idx > 0 {
...
```go include go/bxtree/bxtree.go L1024-L1033
		if tree.summarizer != nil {
...
```go include go/bxtree/bxtree.go L1045-L1050
...
```go include go/bxtree/bxtree.go L1052-L1064
