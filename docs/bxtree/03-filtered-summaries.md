...
[`go/bxtree/bxtree.go:1007`](../../go/bxtree/bxtree.go#L1007)). What the package *does* provide is a
...
| API | Answers | Code |
|---|---|---|
| `FindPath(predicate)` | "the first item whose accumulated summary satisfies a `(acc, cur)` predicate" — O(height + items in one leaf) | [`go/bxtree/bxtree.go:1007-1063`](../../go/bxtree/bxtree.go#L1007-L1063) |
| `Node.SummaryBefore(tree)` | total summary of every item before this leaf | [`go/bxtree/bxtree.go:938-963`](../../go/bxtree/bxtree.go#L938-L963) |
| `Node.Index()` | absolute index of the leaf's first item | [`go/bxtree/bxtree.go:980-998`](../../go/bxtree/bxtree.go#L980-L998) |
| `Node.UpdateSummary` / `Node.UpdateSummaryUpward` | re-accumulate this node's summary (and its ancestors') from scratch | [`go/bxtree/bxtree.go:909-936`](../../go/bxtree/bxtree.go#L909-L936) |
| `All()` / `Reverse()` | full iteration, the substrate for a caller-written filter | [`go/bxtree/bxtree.go:244-279`](../../go/bxtree/bxtree.go#L244-L279) |
...
the `Summarizer[T, S]` interface ([`go/bxtree/types.go:39-50`](../../go/bxtree/types.go#L39-L50)): `FromItem`
...
([`go/bxtree/bxtree.go:658`](../../go/bxtree/bxtree.go#L658))). A leaf's summary is the `FromItem`-fold of its
items ([`go/bxtree/bxtree.go:50-61`](../../go/bxtree/bxtree.go#L50-L61)); a branch's is the `Add`-fold of its
...
([`go/bxtree/bxtree.go:1007`](../../go/bxtree/bxtree.go#L1007)) walks the tree with the predicate over
...
- `acc` is the summary of every item visited so far — it *excludes* the
  ...
  `Add(acc, child.summary)`, [`bxtree.go:1030-1042`](../../go/bxtree/bxtree.go#L1030-L1042)).
- In a branch: the *first child* whose `predicate(acc, child.summary)` is true
  is descended into ([`bxtree.go:1036-1039`](../../go/bxtree/bxtree.go#L1036-L1039)); if no child satisfies, the code
  panics ([`bxtree.go:1044-1046`](../../go/bxtree/bxtree.go#L1044-L1046)) — a caller predicate must be consistent with
  the tree's summary 0-anchoring (see below).
- In a leaf: per *item*, the predicate gets `(acc, FromItem(item))`
  ([`bxtree.go:1050-1053`](../../go/bxtree/bxtree.go#L1050-L1053)). The first satisfying item wins: return
...
- If nothing matches in the leaf, `FindPath` returns `(nil, -1, acc)` — the
  accumulated total ([`bxtree.go:1063`](../../go/bxtree/bxtree.go#L1063)).
- `tree.root == nil` → `(nil, -1, zero)` ([`bxtree.go:1014-1016`](../../go/bxtree/bxtree.go#L1014-L1016)); missing
  summarizer or nil predicate → panic ([`bxtree.go:1018-1020`](../../go/bxtree/bxtree.go#L1018-L1020)).
...
```go include go/bxtree/bxtree.go L1196-L1221
	var acc S
	first := true
	curr := tree.root
...
O(n). The `!found` panic ([`bxtree.go:1045-1046`](../../go/bxtree/bxtree.go#L1045-L1046)) is an *assertion on the
...
`Node.Index()` + `pos` ([`go/bxtree/bxtree.go:981-998`](../../go/bxtree/bxtree.go#L981-L998)).
...
([`go/bxtree/bxtree.go:246-262`](../../go/bxtree/bxtree.go#L246-L262), page 01 § layout); a value-predicate filter
...
```go include go/bxtree/bxtree.go L1083-L1101
...
the root ([`go/bxtree/bxtree.go:929-936`](../../go/bxtree/bxtree.go#L929-L936) — plain `UpdateSummary` at every ancestor).
...
```go include go/bxtree/bxtree.go L1117-L1136
	var s S
	first := true
...
the leaf's end ([`go/bxtree/bxtree.go:962`](../../go/bxtree/bxtree.go#L962)). `Node.Index()`
([`go/bxtree/bxtree.go:980-998`](../../go/bxtree/bxtree.go#L980-L998)) is the same arithmetic counted on `size`, so
...
that ([`go/bxtree/summary_ops_test.go:28-38`](../../go/bxtree/summary_ops_test.go#L28-L38)):
...
[`go/bxtree/summary_ops_test.go:76-105`](../../go/bxtree/summary_ops_test.go#L76-L105) pins the cases a caller should trust:
...
predicate case ([`go/bxtree/summary_ops_test.go:91-98`](../../go/bxtree/summary_ops_test.go#L91-L98) — stops at the first
...
- `verifyNode` ([`go/bxtree/helpers_test.go:164-166`](../../go/bxtree/helpers_test.go#L164-L166) within
  [`go/bxtree/helpers_test.go:125-189`](../../go/bxtree/helpers_test.go#L125-L189)) — summary correctness: a leaf's
  ...
  compute).
- `verifyNode` + `checkNodeBounds` ([`go/bxtree/helpers_test.go:168-186`](../../go/bxtree/helpers_test.go#L168-L186),
  [`go/bxtree/helpers_test.go:193-224`](../../go/bxtree/helpers_test.go#L193-L224)) — occupancy & parent/child edge
  shape.

The driver: `FuzzBxTree` ([`go/bxtree/fuzz_test.go:10-101`](../../go/bxtree/fuzz_test.go#L10-L101)) generates
insert/delete interleavings with and without a summarizer
([`go/bxtree/fuzz_test.go:24-42`](../../go/bxtree/fuzz_test.go#L24-L42)), mirroring the content, and asserts the whole
