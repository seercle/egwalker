...
off that root ([`go/pheap/pheap.go:1-6`](../../go/pheap/pheap.go#L1-L6)). Unlike a binary heap it stores no array and
...
two-pass tournament ([`go/pheap/pheap.go:40-86`](../../go/pheap/pheap.go#L40-L86)). That laziness is why `Push`,
melding, and peeking are O(1) pointer surgery, and why delete-min is the only
operation that ever touches the forest.

The heap is generic (`PairingHeap[T any]`, [`go/pheap/types.go:12-16`](../../go/pheap/types.go#L12-L16)) and
priority is whatever your `less` function says: `a < b` gives a max-heap
(the default, [`go/pheap/pheap.go:92`](../../go/pheap/pheap.go#L92)), `a > b` gives a min-heap. In this
...
`TestSingleElement` in [`go/pheap/pheap_test.go:23-157`](../../go/pheap/pheap_test.go#L23-L157), plus random op-sequence
checking in `FuzzHeap` ([`go/pheap/fuzz_test.go:19`](../../go/pheap/fuzz_test.go#L19)) — both run as part of
