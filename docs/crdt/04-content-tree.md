...
rope ([`go/crdt/content_tree.go:34-40`](../../go/crdt/content_tree.go#L34-L40)) built on the repo's own `bxtree`, and
...
it actually leans on is `content.go`'s interface ([`go/crdt/content.go:13-18`](../../go/crdt/content.go#L13-L18)):
...
same tree serves text (`runeText`, rune-counted, [`go/crdt/content.go:18-20`](../../go/crdt/content.go#L18-L20))
and arrays (`itemRun[T]`, [`go/crdt/content.go:55-66`](../../go/crdt/content.go#L55-L66)) with element types
erased; map runs never reach it (map ops carry no positions and no deleted
content, [`go/crdt/content.go:83-100`](../../go/crdt/content.go#L83-L100)). The trio `(Len, SplitAt, Concat)` is
everything the index math below does; `Collapsible` belongs to the op-log
page's fold fast path (`docs/crdt/02-op-log.md`, [`go/crdt/content.go:75-81`](../../go/crdt/content.go#L75-L81)),
not to the rope.

The tree itself is one embedded `bxtree` whose items are *rope leaves*
([`go/crdt/content_tree.go:34-40`](../../go/crdt/content_tree.go#L34-L40), built once per branch by `newContentTree`,
[`content_tree.go:50-59`](../../go/crdt/content_tree.go#L50-L59)), with each leaf carrying its cached count ([`content_tree.go:12-22`](../../go/crdt/content_tree.go#L12-L22)):
...
unit (`Len()` is literally the root's summary, [`content_tree.go:61-67`](../../go/crdt/content_tree.go#L61-L67)), and
each leaf's count is maintained *arithmetically*: SplitAt's halves have known
lengths at the split point ([`content_tree.go:16-18`](../../go/crdt/content_tree.go#L16-L18)), so the only scan ever
paid is one `run.Len()` at insert time ([`content_tree.go:12-15`](../../go/crdt/content_tree.go#L12-L15)). Two knobs
complete the shape: `ropeLeafCap = 256`
([`content_tree.go:5-10`](../../go/crdt/content_tree.go#L5-L10)) bounds how many characters a single folded leaf
...
([`content_tree.go:42-48`](../../go/crdt/content_tree.go#L42-L48)).

## `locate`: the find descent in character space

Every positional decision in the rope bottoms out in one function
([`go/crdt/content_tree.go:76-85`](../../go/crdt/content_tree.go#L76-L85)):
...
edges). Its contract ([`content_tree.go:72-75`](../../go/crdt/content_tree.go#L72-L75)): descend against summed
...
leaf ([`content_tree.go:84`](../../go/crdt/content_tree.go#L84)), and it panics rather than clamps
([`content_tree.go:80-82`](../../go/crdt/content_tree.go#L80-L82)) — callers validate positions (`Delete` proves it,
[`content_tree.go:209-212`](../../go/crdt/content_tree.go#L209-L212)); the drive-side analog returning its own two-axis
...
`Insert` ([`go/crdt/content_tree.go:134-189`](../../go/crdt/content_tree.go#L134-L189)) has three shapes. The easy two
are `pos == 0` and `pos == n` ([`content_tree.go:150-167`](../../go/crdt/content_tree.go#L150-L167)): fold into the
first/last leaf when the result fits `ropeLeafCap`
([`content_tree.go:152-153`](../../go/crdt/content_tree.go#L152-L153), `:161-162`) — those fold checks are why 5000
single-character appends still leave fewer than 100 leaves
([`go/crdt/content_tree_test.go:152-163`](../../go/crdt/content_tree_test.go#L152-L163)) — otherwise append a fresh leaf.
The interesting one is interior insertion, which *lookahead* resolves by
first locating the landing leaf ([`go/crdt/content_tree.go:169-189`](../../go/crdt/content_tree.go#L169-L189)):
...
[`content_tree.go:181-182`](../../go/crdt/content_tree.go#L181-L182)) — the leaf is cut precisely at the landing
character, after which either the new run fuses with the left half
(`offset+rn <= ropeLeafCap`, [`content_tree.go:184-187`](../../go/crdt/content_tree.go#L184-L187)) or three leaves
result ([`content_tree.go:188`](../../go/crdt/content_tree.go#L188)). All of it funnels into the rope's single
write primitive, `replaceLeaf` ([`go/crdt/content_tree.go:86-104`](../../go/crdt/content_tree.go#L86-L104)):
...
back" ([`content_tree.go:86-104`](../../go/crdt/content_tree.go#L86-L104)) — empty parts are skipped (the
...
([`go/crdt/content_tree.go:105-132`](../../go/crdt/content_tree.go#L105-L132)) — pull the leaf at `idx` into its left
...
`Delete`'s doc comment ([`go/crdt/content_tree.go:191-204`](../../go/crdt/content_tree.go#L191-L204)) names the case
taxonomy; the head handles the cheap exits — invalid/zero-length no-op,
whole-document wipe ([`content_tree.go:205-220`](../../go/crdt/content_tree.go#L205-L220)) — and then locates *both*
edges of the range ([`go/crdt/content_tree.go:222-247`](../../go/crdt/content_tree.go#L222-L247)):
...
([`content_tree.go:234-235`](../../go/crdt/content_tree.go#L234-L235)). The three sub-cases differ only in what seam
...
entirely ([`content_tree.go:222-225`](../../go/crdt/content_tree.go#L222-L225)).
...
([`go/crdt/content_tree.go:249-270`](../../go/crdt/content_tree.go#L249-L270)):
...
([`content_tree.go:250-254`](../../go/crdt/content_tree.go#L250-L254)); the *right* edge is then **re-located**, because
the left split shifted item indices and the only honest bookmark is the
character coordinate ([`content_tree.go:256-258`](../../go/crdt/content_tree.go#L256-L258)); the right leaf is kept
whole when the range ends inside it ([`content_tree.go:259-264`](../../go/crdt/content_tree.go#L259-L264), keeping its
...
([`content_tree.go:266-268`](../../go/crdt/content_tree.go#L266-L268)); and `mergeWithLeft` fuses the leaves the
excision just exposed ([`content_tree.go:269`](../../go/crdt/content_tree.go#L269)) so a delete cannot leave a
...
([`go/crdt/crdt.go:524-538`](../../go/crdt/crdt.go#L524-L538); the full drive walk is
...
- **Local edits take whole-run calls.** `d.Del` applies the author's own
  ...
  ([`document.go:55`](../../go/crdt/document.go#L55)) → `contentTree.Delete(pos, length)`
  ([`content_tree.go:205`](../../go/crdt/content_tree.go#L205)) — and `InsRun`'s `syncRun` does the same for
  inserts ([`document.go:34`](../../go/crdt/document.go#L34)). This is the rope's *cheapest* shape: a range
  ...
  ([`content_tree.go:215-220`](../../go/crdt/content_tree.go#L215-L220)).
- **Remote deletes cannot batch the same way.** The drive targets
  ...
  `deleteOne` returns `-1` for exactly those ([`crdt.go:501-507`](../../go/crdt/crdt.go#L501-L507)); the
  ...
  ([`crdt.go:532-534`](../../go/crdt/crdt.go#L532-L534)). Same content, slower-looking route: the per-char
...
- **Inserts are whole-run on both paths.** Once the drive has integrated an
  ...
  call ([`crdt.go:390-392`](../../go/crdt/crdt.go#L390-L392)), just as the local path does. Position resolution
...
`lv`/`originLeft`/`originRight`/`deleted` fields ([`go/crdt/types.go:82-90`](../../go/crdt/types.go#L82-L90)),
tombstones that stay as items forever ([`crdt.go:509-516`](../../go/crdt/crdt.go#L509-L516) sets `deleted=true`
...
visible cell from the rope ([`crdt.go:534-536`](../../go/crdt/crdt.go#L534-L536)) while the *item* remains in
the item tree. The contentTree is thus a pure *content view*: renderable by
a plain leaf walk ([`content_tree.go:272-280`](../../go/crdt/content_tree.go#L272-L280)), identical on every replica
...
byte-backed string ([`go/crdt/content.go:22-49`](../../go/crdt/content.go#L22-L49)):
...
smaller — `Insert`'s interior path ([`content_tree.go:183-188`](../../go/crdt/content_tree.go#L183-L188)) and `Delete`'s
single-leaf survivors ([`content_tree.go:234-235`](../../go/crdt/content_tree.go#L234-L235)) and edge trims
...
function ([`content_tree_test.go:225-243`](../../go/crdt/content_tree_test.go#L225-L243) pins it; the multibyte storm
attacks it, [`content_tree_test.go:171-221`](../../go/crdt/content_tree_test.go#L171-L221)). Slice-backed `itemRun[T]` cuts
the same way with cheap subslices ([`content.go:58-66`](../../go/crdt/content.go#L58-L66)).
...
`ForEachContent`, [`content_tree.go:272-280`](../../go/crdt/content_tree.go#L272-L280) — the driver outside the package
can only see rendered strings, which is what the transcript above prints
verbatim):

| # | Call | Rope leaves after | Cited mechanics |
|---|------|-------------------|-----------------|
| 1 | `a.Ins(0,"ABCDE")` | a: `[ABCDE]` — 1 leaf | empty tree → `InsertRange(0, run)` ([`content_tree.go:145-148`](../../go/crdt/content_tree.go#L145-L148)) |
| 2 | `b.MergeFrom(a)` | b: `[ABCDE]` — 1 leaf | remote insert is whole-run too: `snapshot.Insert(endPos, "ABCDE")`, `endPos == 0` ([`crdt.go:390-392`](../../go/crdt/crdt.go#L390-L392)) |
| 3 | `a.Ins(2,"xy")` | a: `[ABxy] [CDE]` — 2 leaves | local whole-run `Insert(2, "xy")` ([`document.go:34`](../../go/crdt/document.go#L34)); interior mid-run: `locate(2)` → offset 2 in leaf `ABCDE`; `SplitAt(2)` → `a="AB"`, `b="CDE"`; `offset+rn = 4 ≤ 256` → `replaceLeaf(0, "ABxy", "CDE")` (`content_tree.go:170,183-185`) |
| 4 | `b.MergeFrom(a)` | b: `[ABxy] [CDE]` — 2 leaves | same rope math on the remote side: `findByCurrentPos(2)` first splits the item `ABCDE` at the boundary ([`crdt.go:415-420`](../../go/crdt/crdt.go#L415-L420)), `integrate` places the `xy` item, then `snapshot.Insert(2, "xy")` re-runs the identical `replaceLeaf` step ([`content_tree.go:169-189`](../../go/crdt/content_tree.go#L169-L189)) |
| 5 | `b.Del(3,3)` | b: `[ABxE]` — 1 leaf | local whole-run `Delete(3,3)` ([`document.go:55`](../../go/crdt/document.go#L55)) spanning leaves `[ABxy]`/`[CDE]` — the multi-leaf series traced below |
| 6 | `a.MergeFrom(b)` | a: `[ABxE]` — 1 leaf | remote per-char: b's length-3 delete run (lv 6-8, [`document.go:54`](../../go/crdt/document.go#L54) pushed by `localDelete`) replays as three `DeleteOne` → `Delete(endPos,1)` calls ([`crdt.go:531-536`](../../go/crdt/crdt.go#L531-L536)) — one per single-leaf sub-case, traced below |
...
1. **The rope always equals the naive model** — the direct content tests
   ...
   ([`go/crdt/content_tree_test.go:20-78`](../../go/crdt/content_tree_test.go#L20-L78), `:82-142`) replay every case's ops
   ...
   full-replay-vs-branch comparison ([`document.go:96-101`](../../go/crdt/document.go#L96-L101), page 01 § Check).
2. **The tree never stores an empty leaf** ([`content_tree.go:36-37`](../../go/crdt/content_tree.go#L36-L37)) —
   `replaceLeaf` filters `n == 0` parts ([`content_tree.go:89-95`](../../go/crdt/content_tree.go#L89-L95)), folds and
...
3. **Edit points do not accumulate fragmentation** — boundary folds
   ([`content_tree.go:150-167`](../../go/crdt/content_tree.go#L150-L167)) and consumer-side seam coalescing
   (`mergeWithLeft`, [`content_tree.go:105-132`](../../go/crdt/content_tree.go#L105-L132), called from the seamed
   ...
   leaves ([`content_tree_test.go:152-163`](../../go/crdt/content_tree_test.go#L152-L163)) and a 200-delete multibyte storm
   stays at ≤ `insRuns` ([`content_tree_test.go:171-221`](../../go/crdt/content_tree_test.go#L171-L221));
   `BenchmarkRopeDeleteStorm` reports `leaves-after-storm` so a regression
   is directly visible ([`content_tree_test.go:282-302`](../../go/crdt/content_tree_test.go#L282-L302)).
4. **Positions are in content units and rune-safe** — `runeText.Len` counts
   runes ([`content.go:20`](../../go/crdt/content.go#L20)), `SplitAt` never splits inside a multibyte rune
   ([`content.go:30-49`](../../go/crdt/content.go#L30-L49)), and the multibyte storm plus `TestRuneTextSplitAt`
...
5. **Structure changes never corrupt positioning** — half lengths come from
   ...
   shifted indices ([`content_tree.go:256-258`](../../go/crdt/content_tree.go#L256-L258)); the `ropeLeafNodeSize` knee
   that makes the descent cheap is pinned by `BenchmarkRopeLeafSizeSweep`
   ([`content_tree.go:42-48`](../../go/crdt/content_tree.go#L42-L48), [`content_tree_test.go:368-394`](../../go/crdt/content_tree_test.go#L368-L394)).
