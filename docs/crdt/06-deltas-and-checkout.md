...
`go/crdt/document.go:180-201`), the frame is `EGD1` — magic, version,
eleven columns, zstd (`go/crdt/delta.go:8-22`) — and applying a delta
replays the *same* drive a merge does (`go/crdt/op_log.go:544-560`,
`go/crdt/crdt.go:870-919`), which is why a delta apply must land exactly
where a merge lands.
...
with the two constants (`go/crdt/delta.go:19-22`):
...
character references, not sender-local lvs** (`go/crdt/delta.go:130-142`
...
log (`go/crdt/op_log.go:554-560`). And **there is no Frontier column**:
the receiver never takes the sender's frontier, it folds new history
through its own (`go/crdt/delta.go:15`, `:590-591`).

The one remaining delta-specific field is `SenderVersion`
(`go/crdt/delta.go:16-18`): a coverage table that is *informational &
ignored by the applier* — never folded into `version`, which would claim
ops the receiver does not hold. Built at `go/crdt/delta.go:270-273`,
...
1. **Every kept wire record is fully self-describing**
   (`go/crdt/delta.go:24-32`): op id `(agent, seq)`, span `length`
...
2. **A compacted sender's pre-critical history has no per-op records** —
   ...
   encoder's two shapes (`go/crdt/delta.go:240-268`):
...
per-op coverage row (`go/crdt/delta.go:252-258`); when the sender's log
...
log (`go/crdt/op_log.go:562-584`, § below). The decoder enforces the
shape (`go/crdt/delta.go:621-689`): an anchor record with an empty row
...
that is itself a causally-valid log (`go/crdt/delta.go:44-48`):
...
- **Per-op skip** (`go/crdt/delta.go:62-65`): an ordinary op is excluded
  ...
  contiguity.
- **Anchor inclusion** (`go/crdt/delta.go:51-61`): the anchor record
...
- **Everything else is kept, in log order.** Log order is topological
  (parents always precede children, `go/crdt/delta.go:46-47`), so no
...
(`BenchmarkDeltaAtScale`, `go/crdt/bench_test.go:398`) the whole-log
...
`TestDeltaTraceScale`, `go/crdt/delta_test.go:954`) the empty-since
...
(`log.opAt`/`log.seqAt`, `go/crdt/delta.go:138-139`) turns each internal
edge into a `(agent, endSeq)` character reference; the receiver resolves
them with `resolveParentLV` (`go/crdt/op_log.go:554-560`), the same
mechanism `mergeInto` uses for grafted logs. Wire encoding for the
pairs is zigzag-agent-delta + uvarint seq (`go/crdt/delta.go:223-238`,
`:525-588`).
...
`mergeFrom`'s tail (`go/crdt/document.go:66-74`):
...
(`go/crdt/op_log.go:518-528`) — and at the branch it is
...
`resolveParentLV(p.agent, p.seq)` per pair (`go/crdt/op_log.go:549-561`):
...
follows the loop (`go/crdt/op_log.go:562-584`): it adopts **only into an
...
adopt branch performs for an anchor op (`op_log.go:480-499`,
`docs/crdt/05-binary-and-compaction.md`).

### The document layer: one page of ceremony, three spellings

The exported trio on `RuneDocument` (`go/crdt/document.go:180-201`;
...
(`document.go:368-397`, mirroring `mergeRecursive` at `:449-477`);
...
compacted invariants (`document.go:744-810`, `checkCompacted` at
`:807-809` — the map has no `Check()` of its own, compaction plan F2).
The equivalence oracle `TestDeltaEqualsMerge`
(`go/crdt/delta_test.go:310`) holds them together:
...
path — `go/crdt/view_cache_test.go:32`).
...
(`go/crdt/crdt.go:639-717`; `mergePoint` stacks whole versions and pops
equal version vectors together, `go/crdt/crdt.go:623-637`,
`go/crdt/types.go:110-113`). The core loop, collapsed to the
...
(`compareArrays`, `go/crdt/crdt.go:623-637`); every point reached by
...
(`crdt.go:700-717` — every remaining point is equal). While
descending, every leaf op reached is classified: on the a-side
(the branch's own frontier) it belongs to `sharedOpsSet`,
on the b-side `bOnlyOpsSet` (`crdt.go:705-711`). The remaining
phases just shape *the order*: children are collected within the
delta subgraph (`crdt.go:718-751`), each op's descendant count within it
is computed bottom-up (`crdt.go:753-797`), and phase 4 emits a
deterministic topological order — lightest subtree first — split back
into the `sharedOps` / `bOnlyOps` lists
(`go/crdt/crdt.go:799-864`). All four phases cost `O(delta subgraph)`, so
...
runs *around* it (`go/crdt/crdt.go:870-919`, verbatim):
...
(`crdt.go:877-886`) with `currentVersion` seeded at the common ancestor
(`:884`) — the staged state from which b-only ops wind *forward*. Three
pieces complete the scheme:

- **The placeholder sentinel** (`go/crdt/crdt.go:889-908`): one ghost
  ...
  *before* the branch's own content, "to the left" of everything the
  ...
  refuses to merge anything into it (`crdt.go:242-244`).
...
- **The frontier is advanced only by the b-only phase**
  (`crdt.go:914-918`): `advanceFrontier` (`go/crdt/op_log.go:234-241`)
...
(`go/crdt/document.go:72-73`, `:88-91`). The parameter exists because a
...
`check` at `document.go:96-101` uses plain full `checkout` for its
ground truth).
...
(`document.go:96-101`), and it is the last statement of every
...
1. **Divergence, then two independent deltas converge the pair.** Both
   ...
   the `since[agent] >= seq+length-1` test (`delta.go:62-65`) skips
   nothing yet. After the first apply, b's log holds a's op too, so the
   return frame skips it (`delta.go:62-65`) and keeps only b's own op —
...
2. **Re-delta after convergence is a no-op frame.** Fully converged,
   every op is covered by `since` (`delta.go:62-65`) and the anchor, if
   any, is redundant (`:52-59`) — what remains is the header, the
   SenderVersion column, and nothing else
   (`go/crdt/delta.go:152-273`). The apply-length print of the same
   round in this driver's internal test census (`TestDeltaFrameEmptyLog`,
   `go/crdt/delta_test.go:209`) shows the zero-op frame is legal input
   and does nothing.
...
4. **Compact then delta: the coverage table rides the frame.** The
   ...
   `delta.go:252-258`), adopts it (`op_log.go:480-499` → coverage
...
5. **A compacted sender vs a fully-caught-up peer ships no anchor
   record** (`delta.go:52-59`), but the frame still carries the
   anchorless coverage *row* (`delta.go:259-266`) — and the receiver
   refuses to adopt it (`op_log.go:575-576` guards on
...
(`go/crdt/delta_test.go:76-149`) and `TestDeltaFrameCompacted`
(`go/crdt/delta_test.go:151-207`).)*
...
| 1b | `ab, _ := a.Delta(b.Version())` | 52 B: one op record, run encoded; parents empty (root, `delta.go:52-65` alone decides) |
| 1c | `b.ApplyDelta(ab)` | `"abxy"`, `map[1:1 2:1]`, `Check OK` (`document.go:193-201`) |
| 1d | `ba, _ := b.Delta(a.Version())` | 54 B: since `map[1:1]` covers a's op fully (`delta.go:62-65`), only "xy" is kept — the 54 vs 52 B difference is byte cost, not op count |
| 2 | `re, _ := a.Delta(b.Version())` | zero-op frame, header + SenderVersion only (`delta.go:44-66` keeps nothing; legal input per `TestDeltaFrameEmptyLog`, delta_test.go:209) |
| 3 | `incr, _ := a.Delta(b.Version())` after `a.Ins(2,"z")` | 57 B: all previously-exchanged ops skip (`delta.go:62-65`), only the new op rides; 75 B is the everything-frame for comparison; `b` lands on `"abzxy"`, equal versions |
| 4 | `cb, _ := a.Delta(c.Version())` after `a.Compact()` | 64 B: anchor record row-0 shape (`delta.go:252-258`); `c` adopts, versions equal, `Check OK` |
| 5 | `bb, _ := a.Delta(b.Version())` | 41 B: the anchor is redundant vs b's `since` (`delta.go:52-59`) → no anchor record; but the frame still carries the anchorless coverage row (`delta.go:259-266`) — which b ignores, b holding history (`op_log.go:575`); b unchanged |
...
- **`TestTrace`** (`go/crdt/trace_test.go:121-132`) replays the trace
...
- **`BenchmarkTrace`** (`go/crdt/trace_test.go:134-181`) is the
  performance twin: measured loop replaying all edits, then — *untimed
  epilogue* (`trace_test.go:157-181`) — writing
  ...
  `trace_test.go:79`, so the JSON decode stays out of the profile).
...
`go/crdt/delta_test.go:954`), i.e. ~1.2× the whole-log frame when a
...
(`BenchmarkDeltaAtScale`, `go/crdt/bench_test.go:398`); the per-agent
...
1. **Delta ≡ merge at the endpoint.** The landing path *is* the merge
   landing path — `ingestOp` both (`op_log.go:524-560` vs
   `:531-542`) — so content, version and `Check()` equality is
   property-tested by `TestDeltaEqualsMerge`
   (`go/crdt/delta_test.go:308-351`) and hammered across arbitrary
   interleavings by `FuzzDeltaConvergence`
   (`go/crdt/fuzz_test.go:428-674`; three replicas via interleaved
...
2. **The sender never overstates the receiver's state.** Inclusion is
   ...
   (`delta.go:62-65`), straddlers whole, anchor only when redundant
   (`:52-61`); `SenderVersion` is never folded into `version`
   (`delta.go:16-18`, `:270-273`, `:691-699`) — the receiver would
   otherwise claim ops it does not hold (skip-delivery and
   coverage invariants would both misfire, `op_log.go:524-528`).
3. **Coverage can only bootstrap an empty log.** The anchorless
   log-level table is refused on any non-empty destination
   (`op_log.go:562-584`) — claiming pre-critical history without the
...
4. **Wire parents are receiver-independent.** `(agent, endSeq)`
   references (built at `delta.go:133-142`, resolved at
   `op_log.go:554-560`) make the frame receiver-independent: no
   sender-lv indirection a different log shape could not resolve —
   the delta-format point, spelled out at `delta.go:24-27`.
5. **Fancy replay must equal full replay.** The `nil`-snapshot shared
   ...
   checkout would (`crdt.go:910-918` vs `:613-615`,
   `document.go:96-101`); `Check()` after every `ApplyDelta`
   (`document.go:200`, `:400`) and after the trace (`TestTrace`) stands
   on that equality. `FuzzDeltaFrame` (`go/crdt/fuzz_test.go:764`) and
   the hostile-column battery (`delta_test.go:810-953`) keep the
